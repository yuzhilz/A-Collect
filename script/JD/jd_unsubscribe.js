...
async function goodsMain() {
  ...
  if (goodPageSize * 1 !== 0) {
    ...
    const le =
      Math.ceil($.goodsTotalNum / 20) - 1 >= 0
        ? Math.ceil($.goodsTotalNum / 20) - 1
        : 0;
    ...
    if (followGoods.totalNum > 0) {
      for (let item of followGoods["data"]) {
        ...
        if (
          ...
          console.log(
            `匹配到了您设定的商品--${stopGoods}，不在进行取消关注商品`
          );
          break;
        }
...
async function shopMain() {
  ...
  if (shopPageSize * 1 !== 0) {
    ...
    const le =
      Math.ceil($.shopsTotalNum / 20) - 1 >= 0
        ? Math.ceil($.shopsTotalNum / 20) - 1
        : 0;
    for (let i = 0; i < new Array(le).length; i++) {
      await $.wait(100);
      await unsubscribeShops();
