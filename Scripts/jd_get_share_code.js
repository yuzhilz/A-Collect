                            ...
                            if (data.data.bizCode === 0) {
                                ...
                                $.taskVos.map((item) => {
                                    if (item.taskType === 14) {
                                        ...
                                        // const url = `https://code.chiang.fun/api/v1/jd/factory/create/${item.assistTaskDetailVo.taskToken}`;
                                        // const JdFactory = await get(url);
                                        // console.log(JdFactory);
                                    }
                                });
                            }
                        }
                    ...
                    if (err) {
                        ...
                        if (safeGet(data)) {
                            ...
                            if (data["ret"] === 0) {
                                ...
                                // const url = `http://api.turinglabs.net/api/v1/jd/farm/create/${data.smp}/`;
                                // const JXNC = await get(url);
                                // console.log(JXNC);
...
function get(url) {
    return new Promise(resolve => {
        $.get(taskGetUrl(url), (err, resp, data) => {
            try {
                if (err) {
                    console.log(JSON.stringify(err));
                    $.logErr(err);
